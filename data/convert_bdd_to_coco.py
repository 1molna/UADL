...
def create_coco_lists(input_labels,
    ...
    for im_id, frame in enumerate(grouped_per_frame):
        images_list.append({'id': int(im_id),
                            'width': IMAGE_WIDTH,
                            'height': IMAGE_HEIGHT,
                            'file_name': frame,
                            'license': 1})
        ...
        for annotation in annotations:
            if annotation['category'] in category_keys:
                ...
                bbox_coco = [
                    ...
                    bbox[2] - bbox[0],
                    bbox[3] - bbox[1]]
                annotations_list.append({'image_id': int(im_id),
                                         'id': count,
