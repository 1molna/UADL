...
if args.dataset == 'voc':
    ...
    args.config = 'configs/faster_rcnn/faster_rcnn_r50_fpn_1x_cocoOS_f.py'
...
if args.dataset == 'voc':
    ...
    if args.subset == 'train':
        dataset = build_dataset(cfg.data.trainT)
    elif args.subset == 'val':
        dataset = build_dataset(cfg.data.val)
    elif args.subset == 'test':
        dataset = build_dataset(cfg.data.testOS)
...
score_threshold = 0.1 # only detections with a max softmax above this score are considered valid
total = 0
allResults = {}
...
save_dir = f'./FRCNN/raw/{args.dataset}/{args.subset}'
print(save_dir)
